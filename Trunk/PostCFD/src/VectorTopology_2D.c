...
void SetTopologicalSeedPoints_2D (void) {
	...
	if (distance == NULL) {
		MSG("SetTopologicalSeedPoints_2D: Memory Allocation Failed 2");
		return;
	}
	...
	if ((SeedPoint == NULL) || (SeedCellID == NULL)) {
		MSG("SetTopologicalSeedPoints_2D: Memory Allocation Failed 1");
		return;
	}
	...
	if (NCriticalPoints == 1) {
		...
		if (!firstTop) {	
			...
			if ((CPoint2D.Type == NULL) || (CPoint2D.Cell == NULL)) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 1");
				return 1;
			}
			
			CPoint2D.CriticalPoint = (Point2 *)malloc(10 * sizeof(Point2));
			if (CPoint2D.CriticalPoint == NULL) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 2");
				return 1;
			}
			...
			if ((CPoint2D.EigenVector1 == NULL) || (CPoint2D.EigenVector2 == NULL)) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 3");
				return 1;
			}
		}
	...
	if ((NCriticalPoints != CPoint2D.Size) && (NCriticalPoints > CPoint2D.Size)) {
		if (NCriticalPoints > (flagCP * 10)) {
			...
			if ((tmp1 == NULL) || (tmp2 == NULL)) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 4");
				return 1;
			}
			CPoint2D.Type = tmp1;
			CPoint2D.Cell = tmp2;
			...
			if (tmp3 == NULL) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 5");
				return 1;
			}
			...
			if (tmp4 == NULL) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 6");
				return 1;
			}
			...
			if (tmp5 == NULL) {
				MSG("AddCriticalPoint2D: Memory Allocation Failed 7");
				return 1;
			}
			...
			switch (Solution2D.Location) {
				...
				case CellCenter:
					...
					if((TopGradVecXX.Data == NULL) || (TopGradVecXY.Data == NULL)) {
						MSG("VectorTopology_2D: Memory Allocation Failed: 1");
						return;
					}
					/* Calculate Gradient */
					...
					if((TopGradVecYX.Data == NULL) || (TopGradVecYY.Data == NULL)) {
						MSG("VectorTopology_2D: Memory Allocation Failed: 2");
						return;
					}
					/* Calculate Gradient */
					...
					*/
					...
					if ((TopVecX.Data == NULL) || (TopVecY.Data == NULL)) {
						MSG("VectorTopology_2D: Memory Allocation Failed");
						return;
					}
					*/
					/* Interpolate */
