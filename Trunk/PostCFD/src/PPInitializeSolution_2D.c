...
int SolutionCC2NodeSelect_2D(void) {
    ...
    switch (CC2NodeFlag) {
        ...
        default:
            /* Error */
            CC2NodeFlag = 0;
            Warn("Invalid Option: Continuing with no interpolation");
    }
    return 0;
}
...
int InitializeSolutionCGNS_2D(ZONE *P) {
    ...
    for (sol = P->sols, ns = 1; ns <= P->nsols; sol++, ns++) {
        if (ns >= 2) {
            Warn("InitializeSolutionCGNS_2D: Only Single Solution Node Supported");
            return 1;
        }
        ...
        if (SolutionCGNSFlag == 0) {
            ...
            if (SolutionCGNS2D.SolCGNS == NULL) {
                Warn("InitializeSolutionCGNS_2D: Memory Failed");
                return 1;
            }
            ...
            switch (sol->location) {
                ...
                case CellCenter:
                    ...
                    for (fld = sol->flds, nf = 1; nf <= sol->nflds; fld++, nf++) {
                        ...
                        if (CC2NodeFlag == 0) {
                            ...
                            if (tmp.Data == NULL) {
                                Warn("InitializSolutionCGNS_2D: Memory Failed");
                                return 1;
                            }
                ...
                default:
                    ...
                    Warn("Unsupported Solution Location");
                    return 1;
            }
...
int InitializeSolution_2D(void) {
    ...
    if (SolutionFlag == 0) {
        ...
        if (Solution2D.Sols == NULL) {
            Warn("IntializeSolution_2D: Memory Failed");
            return 1;
        }
...
int UpdateSolution_2D(Data_2D_Un *newsol) {
    ...
    if (tmp == NULL) {
        Warn("UpdateSolution_2D: Memory Failed: 1");
        return 1;
    }
    ...
    Solution2D.Sols = (Data_2D_Un **) malloc(Solution2D.Size * sizeof (Data_2D_Un *));
    if (Solution2D.Sols == NULL) {
        Warn("UpdateSolution_2D: Memory Failed: 2");
        return 1;
    }
