...
void GetSeedPoints_2D (void) {
	...
	if ((SeedPoint == NULL) || (SeedCellID == NULL)) {
		MSG("GetSeedPoint_2D: Memory Failed");
		StreamlineFlag = 0;
		return;
	}
...
int CellCononicalTransform_2D (void) {
	...
	if (Cell2D[0].NodesPerCell != 3) {
		MSG("CellCononicalTransform_2D: Topology Not Supported");
		return 1;
	}
	...
	if (CellConTran == NULL) {
		MSG("CellCononicalTransform_2D: Memory Failed 1");
		free(CellConTran);
		return 1;
	}
	...
	if (CellConConst == NULL) {
		MSG("CellCononicalTransform_2D: Memory Failed 2");
		free(CellConConst);
		free(CellConTran);
		return 1;
	}
...
void StreamTimeStep_2D (void) {
	...
	if (Cell2D[0].NodesPerCell != 3) {
		MSG("StreamTimeStep_2D: Topology Not Supported");
		return;
	}
		...
		switch (Solution2D.Location) {
			...
			case CellCenter:
				...
				if (StreamVelX.Data == NULL) {
					MSG("StreamTimeStep_2D: Memory Failed 1");
					return;
				}
				...
				if (StreamVelY.Data == NULL) {
					MSG("StreamTimeStep_2D: Memory Failed 2");
					return;
				}
...
void StreamEqCoef_2D (void) {
	...
	if (Cell2D[0].NodesPerCell != 3) {
		MSG("StreamEqCoeff_2D: Topology Not Supported");
		return;
	}
	...
	if ((StreamCoef == NULL) || (StreamConst == NULL)) {
		MSG("StreamEqCoef_2D: Memory Failed");
		return;
	}
...
void Streamline_2D (void) {
	...
	switch (StreamModule) {
		case 1:
			...
			if ((StreamlineArg1 == -1) || (StreamlineArg2 == -1)) {
				MSG("Streamline_2D: Invalid Vector Initialization");
				return;
			}
	...
	/* Check Seed Points */
	if (NSeedPoint != SeedID) {
		MSG("Streamline_2D: Seed Points Mismatched");
		return;
	}
	...
	/* Check Stream Time */
	if(StreamTime == 0.0) {
		MSG("Streamline_2D: Stream Time = 0.0");
		return;
	}
	...
	if (i > 100000) {
		MSG("Streamline_2D: StreamTime too large");
		return;
	}
