			...
			case 4:
				...
				if (CrossO_2D(&D, &A, &B) < 0.0) {
					if (CrossO_2D(&D, &A, &C) > 0.0) {
						...
						MSG("CounterClockCellNodes_2D: Debug Algo:1");
						return 1;
					}	
				}
				else {
					if (CrossO_2D(&D, &A, &C) > 0.0) {
						MSG("CounterClockCellNodes_2D: Debug Algo:2");
						return 1;
					}
					else {
						for (i = 1; i < 4; i++) {
	...
	for (icell = 0; icell < NoCells2D; icell++) {
		...
		switch (nnodes) {
			case 3:
				...
				if (vector == NULL) {
					MSG("Memory Allocation Failed for vector");
					return 1;
				}
