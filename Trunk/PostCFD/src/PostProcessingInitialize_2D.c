...
int compress_sort(int *A, int value, int size) {
	...
	if (data == NULL) {
		MSG("compress_sort: Memory Allocation failed for data");
		return 1;
	}
	...
	if (j != count) {
		MSG("compress_sort: Miss-Matched count and j");
		return 1;
	}
...
int GetCellAdjacentCellsQuad_2D (int cellid) {
	...
	if (data == NULL) {
		MSG("GetCellAdjacentCellsQuad_2D: Memory Allocation Failed 1");
		return 1;
	}
	...
	if (Cell2D[cellid].NearNeighbours == NULL) {
		MSG("GetCellAdjacentCellsQuad_2D: Memory Allocation Failed 2");
		return 1;
	}
...
int GetCellAdjacentCellsTri_2D (int cellid) {
	...
	data = (int *) malloc (size * sizeof (int));
	if (data == NULL) {
		MSG("GetCellAdjacentCellsTri_2D: Memory Allocation Failed 1");
		return 1;
	}
	...
	Cell2D[cellid].NearNeighbours = (int *) malloc (count * sizeof (int));
	if (Cell2D[cellid].NearNeighbours == NULL) {
		MSG("GetCellAdjacentCellsTri_2D: Memory Allocation Failed 2");
		return 1;
	}
...
int CreateSymmetricCSRFormat_2D (void) {
	...
	if(IA_2D == NULL) {
		MSG("CreateSymmetricCSRFormat_2D: Memory Allocation Failed 1");
		return 1;
	}
	...
	if(JA_2D == NULL) {
		MSG("CreateSymmetricCSRFormat_2D: Memory Allocation Failed 2");
		return 1;
	}
...
int CreateNodeAdjacentCells_2D (void) {
	...
	if (NodeAdjacentCells2D == NULL) {
		MSG("CreateNodeAdjacentCells_2D: Memory Allocation Failed");
		return 1;
	}
...
int SearchInsert_2D (int minnode, int maxnode) {
	...
	if (minnode > maxnode) {
		MSG("SearchInsert_2D: Node Order Is Wrong");
		return 1;
	}
...
int CreateNodeAdjacentNodes_2D (void) {
	...
	if (NodeAdjacentNodes2D == NULL) {
		MSG("CreateNodeAdjacentNodes_2D: Memory Allocation Failed");
		return 1;
	}
...
int CreatEdgeStructure_2D (void) {
	...
	if (Edge2D == NULL) {
		MSG("CreatEdgeStructure_2D: Memory Allocation Failed");
		return 1;
	}
...
int InitializeUnstructuredGrid_2D (ZONE *P) {
	...
	if (Node2D == NULL) {
		MSG("InitializeUnstructuredGrid_2D: Memory Allocation Failed");
		return 1;
	}
	...
	for (eset = P->esets, e = 1; e <= P->nesets; e++, eset++) {
		if (eset->start != 1 || eset->end != NoCells2D) {
			MSG("InitializeUnstructuredGrid_2D: Cell Connectivity Miss-Matched");
			return 1;
		}
		
		if (eset->type != TRI_3) {
			MSG("InitializeUnstructuredGrid_2D: Not A Triangluar Grid");
			return 1;
		}
		...
		if (Cell2D == NULL) {
			MSG("InitializeUnstructuredGrid_2D: Memory Allocation Failed 2");
			return 1;
		}
	...
	if (P->nbocos != 0) {
		...
		MSG("InitializeUnstructuredGrid_2D: Boundary Condition Missing");
	}
