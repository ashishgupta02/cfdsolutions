...
void Compute_Characteristic_BoundaryCondition(int BEdgeID, int Iteration) {
    ...
    switch (bndEdge[BEdgeID].type) {
        ...
        case BC_SOLID_WALL:
            ...
            if (Iteration >= ZPGIteration) {
                if ((lamda5 > 0.0) || (lamda4 < 0.0)) {
                    ...
                    warn("Compute_Characteristic_BoundaryCondition: Unable apply Solid Wall boundary condition edge[%d] - 1", BEdgeID);
                }
            }
            break;
        // Free Stream
        case BC_FREE_STREAM:
            ...
            if ((lamda1 <= 0.0) && (lamda4 <= 0.0) && (lamda5 <= 0.0))
                instance = BC_FREE_STREAM_SUPERSONIC_INFLOW;
            // Supersonic outflow
            else if ((lamda1 >= 0.0) && (lamda4 >= 0.0) && (lamda5 >= 0.0))
                instance = BC_FREE_STREAM_SUPERSONIC_OUTFLOW;
            // Subsonic outflow
            else if ((lamda1 >= 0.0) && (lamda5 <= 0.0) && (lamda4 >= 0.0))
                instance = BC_FREE_STREAM_SUBSONIC_OUTFLOW;
            // Subsonic inflow
            else if ((lamda1 <= 0.0) && (lamda4 >= 0.0) && (lamda5 <= 0.0))
                instance = BC_FREE_STREAM_SUBSONIC_INFLOW;
            else {
    ...
    switch (instance) {
        ...
        case BC_SUPERSONIC_OUTFLOW:
            ...
            w_b_n   = w_i;
            break;
        case BC_FREE_STREAM_SUBSONIC_OUTFLOW:
            p_b_n   = Inf_Pressure + Gauge_Pressure;
            rho_b_n = rho_i + (p_b_n - p_i) / (c_0 * c_0);
            u_b_n   = u_i - nx * (p_b_n - p_i) / (rho_0 * c_0);
            v_b_n   = v_i - ny * (p_b_n - p_i) / (rho_0 * c_0);
            w_b_n   = w_i - nz * (p_b_n - p_i) / (rho_0 * c_0);
            break;
        case BC_FREE_STREAM_SUBSONIC_INFLOW:
            ...
            break;
        case BC_FREE_STREAM_SUPERSONIC_INFLOW:
            p_b_n   = Inf_Pressure + Gauge_Pressure;
            rho_b_n = Inf_Rho;
            u_b_n   = Inf_U;
            v_b_n   = Inf_V;
            w_b_n   = Inf_W;
            break;
        case BC_FREE_STREAM_SUPERSONIC_OUTFLOW:
            p_b_n   = p_i;
            rho_b_n = rho_i;
            u_b_n   = u_i;
            v_b_n   = v_i;
        ...
        case BC_SOLID_WALL:
            ...
            if (Iteration >= ZPGIteration) {
                if ((lamda5 > 0.0) || (lamda4 < 0.0)) {
                    ...
                    info("Normal: %lf %lf %lf ", nx, ny, nz);
                    info("Velocity: %lf %lf %lf ", u_i, v_i, w_i);
                    warn("Compute_Characteristic_BoundaryCondition_New: Unable apply Solid Wall boundary condition - 1");
                }
            }
            break;
        // Free Stream
...
int Compute_Precondition_Characteristic_BoundaryCondition(double Q_L[NEQUATIONS], double Q_R[NEQUATIONS], 
    ...
    if (PrecondMethod == SOLVER_PRECOND_ROE_CV || PrecondMethod == SOLVER_PRECOND_ROE_CV_ORIGINAL) {
        ...
        case BC_SOLID_WALL:
            ...
            if (Iteration >= ZPGIteration) {
                if ((lamda5 > 0.0) || (lamda4 < 0.0)) {
                    ...
                    info("Normal: %lf %lf %lf ", nx, ny, nz);
                    info("Velocity: %lf %lf %lf ", u_i, v_i, w_i);
                    warn("Compute_Precondition_Characteristic_BoundaryCondition: Unable apply Solid Wall boundary condition - 1");
                }
            }
            break;
        // Free Stream
...
//------------------------------------------------------------------------------
//! Solid Wall Boundary Condition is Pressure Based Hence Weak Formulation
//! Note: Outward Pointing Boundary Normals
//------------------------------------------------------------------------------
        ...
        case BC_SOLID_WALL:
            ...
            if (Iteration >= ZPGIteration) {
                if ((lamda5 > 0.0) || (lamda4 < 0.0)) {
                    ...
                    info("Normal: %lf %lf %lf ", nx, ny, nz);
                    info("Velocity: %lf %lf %lf ", u_i, v_i, w_i);
                    warn("Compute_Characteristic_Weak_BoundaryCondition: Unable apply Solid Wall boundary condition - 1");
                }
            }
            break;
        // Free Stream
...
void Apply_Boundary_Condition(int Iteration) {
    switch (SolverBCScheme) {
        case SOLVER_BC_SCHEME_CHARCTERISTIC: // Characteristic Boundary Condition
            ...
            break;
        case SOLVER_BC_SCHEME_CHARCTERISTIC_WEAK: // Characteristic Weak Boundary Condition
