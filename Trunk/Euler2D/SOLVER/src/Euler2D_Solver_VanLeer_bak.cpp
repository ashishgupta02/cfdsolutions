...
void Euler2D_Solver_VanLeer::Init() {
    ...
    CFL_Min                = 0.0;
    CFL_Max                = 0.0;
    BlockMatrix.VectorSize = 0;
    BlockMatrix.BlockSize  = 0;
    BlockMatrix.CRSSize    = 0;
    BlockMatrix.A          = NULL;
    BlockMatrix.B          = NULL;
    BlockMatrix.IA         = NULL;
...
Euler2D_Solver_VanLeer::~Euler2D_Solver_VanLeer() {
    ...
    if (BlockMatrix.A != NULL) {
        for (i = 0; i < BlockMatrix.CRSSize; i++) {
            if (BlockMatrix.A[i] != NULL) {
                for (j = 0; j < BlockMatrix.BlockSize; j++) {
    ...
    if (BlockMatrix.B != NULL) {
        for (i = 0; i < BlockMatrix.VectorSize ; i++) {
...
void Euler2D_Solver_VanLeer::Create_CRS_BlockMatrix() {
    ...
    BlockMatrix.VectorSize = mesh.nnodes;
    BlockMatrix.BlockSize  = 4;
    ...
    BlockMatrix.IA[0] = 0;
    for (i = 0; i < mesh.nnodes; i++) {
        BlockMatrix.CRSSize += degree[i] + 1;
    ...
    BlockMatrix.JA = NULL;
    BlockMatrix.JA = new int[BlockMatrix.CRSSize];
#ifdef DEBUG
    if (BlockMatrix.JA == NULL)
        error("Create_CRS_BlockMatrix: %s\n", "Error Allocating Memory 4");
#endif
    ...
    BlockMatrix.A = NULL;
    BlockMatrix.A = (double ***) malloc (BlockMatrix.CRSSize*sizeof(double**));
#ifdef DEBUG
    if (BlockMatrix.A == NULL)
    ...
    for (i = 0; i < BlockMatrix.CRSSize; i++) {
        ...
        BlockMatrix.A[i] = (double **) malloc (BlockMatrix.BlockSize*sizeof(double*));
#ifdef DEBUG
        if (BlockMatrix.A[i] == NULL)
        ...
        for (j = 0; j < BlockMatrix.BlockSize; j++) {
            ...
            BlockMatrix.A[i][j] = (double *) malloc (BlockMatrix.BlockSize*sizeof(double));
#ifdef DEBUG
            if (BlockMatrix.A[i] == NULL)
                error("Create_CRS_BlockMatrix: %s\n", "Error Allocating Memory 7");
#endif
            for (k = 0; k < BlockMatrix.BlockSize; k++)
    ...
    BlockMatrix.B = NULL;
    BlockMatrix.B = (double **) malloc (BlockMatrix.VectorSize*sizeof(double*));
#ifdef DEBUG
    if (BlockMatrix.B == NULL)
    ...
    for (i = 0; i < BlockMatrix.VectorSize; i++) {
        ...
        BlockMatrix.B[i] = (double *) malloc (BlockMatrix.BlockSize*sizeof(double));
#ifdef DEBUG
        if (BlockMatrix.B[i] == NULL)
    ...
    BlockMatrix.X = NULL;
    BlockMatrix.X = (double **) malloc (BlockMatrix.VectorSize*sizeof(double*));
#ifdef DEBUG
    if (BlockMatrix.X == NULL)
    ...
    for (i = 0; i < BlockMatrix.VectorSize; i++) {
        ...
        BlockMatrix.X[i] = (double *) malloc (BlockMatrix.BlockSize*sizeof(double));
#ifdef DEBUG
        if (BlockMatrix.X[i] == NULL)
